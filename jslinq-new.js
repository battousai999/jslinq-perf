...
class LinqInternal
{
    ...
    static buildZipGenerator(firstIterator, secondIterator, resultSelector)
    {
        return function* ()
        {
...
export class Linq
{
    ...
    zip(second, resultSelector)
    {
        ...
        if (Linq.isArray(this.source) && Linq.isArray(second))
        {
            let len = Math.min(this.source.length, second.length);
            let results = [];

            for (let i = 0; i < len; i++)
            {
                results.push(resultSelector(this.source[i], second[i]));
            }
        ...
        else
        {
            ...
            return new Linq(LinqInternal.buildZipGenerator(firstIterator, secondIterator, resultSelector));
        }
    }
